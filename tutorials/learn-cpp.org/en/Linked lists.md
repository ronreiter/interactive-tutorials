...
and one pointer. The pointer always points to the next member of the list. If the pointer is nullptr, then it is
the last node in the list.

A linked list is held using a local pointer variable which points to the first item of the list. If that pointer
is also nullptr, then the list is considered to be empty.
...
pointer to the next node, and print again, until we've reached the end of the list (the next node is nullptr).
...
-------------
    ...
    int main() {
        ...
        Node * test_list = new Node;
        test_list->val = 1;
        test_list->next = new Node;
        test_list->next->val = 2;
        test_list->next->next = new Node;
        test_list->next->next->val = 3;
        test_list->next->next->next = new Node;
        test_list->next->next->next->val = 4;
        test_list->next->next->next->next = nullptr;
