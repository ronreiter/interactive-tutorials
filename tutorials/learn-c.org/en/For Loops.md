...
For example, if we wish to iterate on a block 10 times, we write:
...
For loops can iterate on array values. For example, if we would want to sum all the values of an array, we could use the iterator i as the array index:
```
