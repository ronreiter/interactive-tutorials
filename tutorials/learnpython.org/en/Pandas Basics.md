...
Exercise
--------
Create a dictionary with columns `country` and `capital`, build a DataFrame named `brics` from it, and print the `capital` column as a Pandas Series.
...
-------------
    ...
    # Create a DataFrame brics from the dict

    # Print out the capital column

Expected Output
---------------
    test_output_contains("Brasilia")
    test_output_contains("Moscow")
    test_output_contains("New Delhi")
