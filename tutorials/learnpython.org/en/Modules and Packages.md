...
file, which could be edited separately.

### Writing modules

Modules in Python are simply Python files with a .py extension. The name of the module is the same as the file name.
...
When the `import draw` directive runs, the Python interpreter looks for a file in the directory in which the script was executed, by the name of the module with a `.py` suffix. In this case it will look for `draw.py`. If it is found, it will be imported. If it's not found, it will continue looking for built-in modules.
...
A namespace is a system where every object is named and can be accessed in Python. We may import the function `draw_game` directly into the main script's namespace, by using the `from` command.
...
You may have noticed that in this example, the name of the module does not precede `draw_game`, because we've specified the module name in the `import` command.
...
Use the `import *` command to import all the objects from a module like this:
...
shorter, and doesn't require you to specify which objects we want to import from the module.


### Custom import name

We may load modules under any name we want. This is useful when importing a module conditionally
...
module once. If another module in your code imports the same module again, it will not be loaded again, so local variables inside the module act as a "singleton." They are initialized only once.

This is useful to know, because this means you can use this to initialize objects. 
For example:
...
default local directory and built-in modules. You can use the environment
variable `PYTHONPATH` to specify additional directories to look for modules like this:
...
This adds the `foo` directory to the list of paths where modules are looked for.
