Tutoriel
--------

### Pandas DataFrames

Pandas est un outil de manipulation de données de haut niveau développé par Wes McKinney. Il est construit sur le package Numpy et sa structure de données clé est appelée DataFrame. Les DataFrames vous permettent de stocker et de manipuler des données tabulaires sous forme de lignes d'observations et de colonnes de variables.

Il existe plusieurs façons de créer un DataFrame. Une façon est d'utiliser un dictionnaire. Par exemple :

<div data-datacamp-exercise data-height="300" data-encoded="true">

</div>

Comme vous pouvez le voir avec le nouveau DataFrame `brics`, Pandas a attribué une clé pour chaque pays sous forme de valeurs numériques allant de 0 à 4. Si vous souhaitez avoir des valeurs d'index différentes, disons, le code pays à deux lettres, vous pouvez facilement le faire également.

<div data-datacamp-exercise data-height="225" data-encoded="true">

</div>

Une autre façon de créer un DataFrame est d'importer un fichier csv à l'aide de Pandas. Maintenant, le csv `cars.csv` est stocké et peut être importé en utilisant `pd.read_csv` :

<div data-datacamp-exercise data-height="300" data-encoded="true">

</div>

### Indexation des DataFrames

Il existe plusieurs façons d'indexer un DataFrame Pandas. L'une des façons les plus simples de le faire est d'utiliser la notation entre crochets.

Dans l'exemple ci-dessous, vous pouvez utiliser des crochets pour sélectionner une colonne du DataFrame `cars`. Vous pouvez utiliser soit un simple crochet soit un double crochet. Le simple crochet renverra une Série Pandas, tandis que le double crochet renverra un DataFrame Pandas.

<div data-datacamp-exercise data-height="300" data-encoded="true">

</div>

Les crochets peuvent également être utilisés pour accéder aux observations (lignes) d'un DataFrame. Par exemple :

<div data-datacamp-exercise data-height="300" data-encoded="true">

</div>

Vous pouvez également utiliser `loc` et `iloc` pour effectuer à peu près n'importe quelle opération de sélection de données. `loc` est basé sur les étiquettes, ce qui signifie que vous devez spécifier les lignes et les colonnes en fonction de leurs étiquettes de ligne et de colonne. `iloc` est basé sur l'index entier, vous devez donc spécifier les lignes et les colonnes par leur index entier comme vous l'avez fait dans l'exercice précédent.

<div data-datacamp-exercise data-height="300" data-encoded="true">

</div>.

Tutorial Code
-------------

Expected Output
---------------

Solution
--------