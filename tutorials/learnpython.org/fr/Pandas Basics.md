...
Comme vous pouvez le voir avec le nouveau DataFrame `brics`, Pandas a attribué une clé pour chaque pays sous forme de valeurs numériques de 0 à 4. Si vous souhaitez avoir des valeurs d'index différentes, par exemple, le code à deux lettres du pays, vous pouvez facilement le faire aussi.
...
Dans l'exemple ci-dessous, vous pouvez utiliser des crochets pour sélectionner une colonne du DataFrame `cars`. Vous pouvez utiliser un crochet simple ou double. Le crochet simple produira une série Pandas, tandis que le double crochet produira un DataFrame Pandas.
...
Les crochets peuvent également être utilisés pour accéder à des observations (lignes) d'un DataFrame. Par exemple:
