...
Comme vous pouvez le voir avec le nouveau DataFrame `brics`, Pandas a attribué une clé pour chaque pays avec les valeurs numériques de 0 à 4. Si vous souhaitez avoir des valeurs d'index différentes, par exemple le code pays à deux lettres, vous pouvez facilement le faire.
...
Une autre façon de créer un DataFrame est d'importer un fichier csv à l'aide de Pandas. Maintenant, le csv `cars.csv` est stocké et peut être importé en utilisant `pd.read_csv` :
...
Dans l'exemple ci-dessous, vous pouvez utiliser des crochets pour sélectionner une colonne du DataFrame `cars`. Vous pouvez utiliser un seul crochet ou un double crochet. Le crochet simple produira une série Pandas, tandis qu'un double crochet produira un DataFrame Pandas.
...
Vous pouvez également utiliser `loc` et `iloc` pour effectuer à peu près n'importe quelle opération de sélection de données. `loc` est basé sur des labels, ce qui signifie que vous devez spécifier les lignes et les colonnes en fonction de leurs labels de ligne et de colonne. `iloc` est basé sur des index entiers, vous devez donc spécifier les lignes et les colonnes par leur index entier comme vous l'avez fait dans l'exercice précédent.
