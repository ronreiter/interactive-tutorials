...
Comme vous pouvez le voir avec le nouveau DataFrame `brics`, Pandas a attribué une clé pour chaque pays sous forme de valeurs numériques de 0 à 4. Si vous souhaitez avoir des valeurs d'index différentes, disons, le code pays à deux lettres, vous pouvez le faire facilement aussi.
...
Une autre manière de créer un DataFrame est d'importer un fichier csv avec Pandas. Désormais, le csv `cars.csv` est stocké et peut être importé à l'aide de `pd.read_csv` :
...
Dans l'exemple ci-dessous, vous pouvez utiliser des crochets pour sélectionner une colonne du DataFrame `cars`. Vous pouvez utiliser un seul crochet ou un double crochet. Le simple crochet renverra une série Pandas, tandis que le double crochet renverra un DataFrame Pandas.
