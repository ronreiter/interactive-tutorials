Tutorial
--------

### Commencer

Les tableaux Numpy sont de superbes alternatives aux listes Python. Certains des principaux avantages des tableaux Numpy sont qu'ils sont rapides, faciles à utiliser, et offrent aux utilisateurs la possibilité de réaliser des calculs sur l'intégralité des tableaux.

Dans l'exemple suivant, vous allez d'abord créer deux listes Python. Ensuite, vous allez importer le paquet numpy et créer des tableaux numpy à partir des listes nouvellement créées.

	# Créez 2 nouvelles listes height et weight
	height = [1.87,  1.87, 1.82, 1.91, 1.90, 1.85]
	weight = [81.65, 97.52, 95.25, 92.98, 86.18, 88.45]

	# Importez le paquet numpy en tant que np
	import numpy as np

	# Créez 2 tableaux numpy à partir de height et weight
	np_height = np.array(height)
	np_weight = np.array(weight)

# Imprimez le type de np_height

    print(type(np_height))

### Calculs élément par élément

Nous pouvons maintenant effectuer des calculs élément par élément sur height et weight. Par exemple, vous pourriez prendre les 6 observations de height et weight ci-dessus, et calculer l'IMC pour chaque observation avec une seule équation. Ces opérations sont très rapides et efficaces en termes de calcul. Elles sont particulièrement utiles lorsque vous avez des milliers d'observations dans vos données.

<div data-datacamp-exercise data-height="225" data-encoded="true">

</div>

### Sous-ensemble

Une autre grande fonctionnalité des tableaux Numpy est la capacité de créer des sous-ensembles. Par exemple, si vous vouliez savoir quelles observations dans notre tableau BMI sont supérieures à 23, nous pourrions rapidement le sous-ensemble pour le découvrir.

<div data-datacamp-exercise data-height="225" data-encoded="true">

</div>

Exercice
--------

Convertissez d'abord la liste de poids d'une liste à un tableau Numpy. Ensuite, convertissez tous les poids de kilogrammes en livres. Utilisez la conversion scalaire de 2,2 livres par kilogramme pour effectuer votre conversion. Enfin, imprimez le tableau résultant de poids en livres.

<div data-datacamp-exercise data-height="300" data-encoded="true">

</div>

Tutorial Code
-------------

Expected Output
---------------

Solution
--------