Tutoriel
--------

### Prise en main

Les tableaux Numpy sont d'excellentes alternatives aux listes Python. Certains des principaux avantages des tableaux Numpy sont qu'ils sont rapides, faciles à utiliser et donnent aux utilisateurs la possibilité d'effectuer des calculs sur des tableaux entiers.

Dans l'exemple suivant, vous allez d'abord créer deux listes Python. Ensuite, vous allez importer le package numpy et créer des tableaux numpy à partir des listes nouvellement créées.

```python
# Create 2 new lists height and weight
height = [1.87,  1.87, 1.82, 1.91, 1.90, 1.85]
weight = [81.65, 97.52, 95.25, 92.98, 86.18, 88.45]

# Import the numpy package as np
import numpy as np

# Create 2 numpy arrays from height and weight
np_height = np.array(height)
np_weight = np.array(weight)

# Print out the type of np_height
print(type(np_height))
```

### Calculs élément par élément

Nous pouvons maintenant effectuer des calculs élément par élément sur la taille et le poids. Par exemple, vous pouvez prendre les 6 tailles et poids observés ci-dessus et calculer l'IMC pour chaque observation avec une seule équation. Ces opérations sont très rapides et efficaces sur le plan computationnel. Elles sont particulièrement utiles lorsque vous avez des milliers d'observations dans vos données.

<div data-datacamp-exercise data-height="225" data-encoded="true">

</div>

### Sous-ensemble

Une autre fonctionnalité intéressante des tableaux Numpy est la possibilité de sous-ensemble. Par exemple, si vous souhaitez savoir quelles observations dans notre tableau d'IMC sont supérieures à 23, nous pourrions rapidement les diviser en sous-ensembles pour le découvrir.

<div data-datacamp-exercise data-height="225" data-encoded="true">

</div>

Exercice
--------

Tout d'abord, convertissez la liste de poids d'une liste en un tableau Numpy. Ensuite, convertissez tous les poids de kilogrammes en livres. Utilisez la conversion scalaire de 2,2 lbs par kilogramme pour effectuer votre conversion. Enfin, imprimez le tableau résultant des poids en livres.

<div data-datacamp-exercise data-height="300" data-encoded="true">

</div>

Tutorial Code
-------------

Expected Output
---------------

Solution
--------